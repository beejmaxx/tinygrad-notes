...
> Historical API: current tinygrad no longer contains the `tinygrad.shape` modules used below. See [the current walkthrough](current-tinygrad.md#where-did-shapetracker-go) for the present source locations. The original explanation is retained for its treatment of indexing and strides.
...
you can reconstruct the full tensor by indexing into the correct position in its linear memory data layout. This isn't
always the case with certain transformations though. Suppose we have a matrix of shape 3 by 2, then we transpose it,
...
`ridx1` refers to 1st dim, ranging from 0 to 1 (shape is 2). And we can compute all the combinations between the two variables
