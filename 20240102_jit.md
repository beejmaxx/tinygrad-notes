...
> Historical implementation. See [current TinyJit capture and replay](current-tinygrad.md#what-tinyjit-captures-now) for a tested example and the present execution model. The compilation/pipeline analogy below should not be read as the current definition of TinyJit.
